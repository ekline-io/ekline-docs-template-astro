...
Every setting in this site — what it does, and what happens if you leave it alone — is documented at <https://potluck.ekline.io>. Its Internals section is the same material as `wiki/`, which shipped with your copy.
...
- The build is adapter-based: `@astrojs/vercel` on Vercel (`VERCEL=1`), `@astrojs/node` everywhere else. Either way the static files land in `dist/client/` and the server bundle in `dist/server/` — never a flat `dist/`. A Vercel build then *copies* `dist/client/` into `.vercel/output/static/`, which is what the deployment serves, so it leaves both populated rather than replacing one with the other. Tests resolve the right one via `tests/helpers/static-dir.mjs`; don't hardcode a path.
