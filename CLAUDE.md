...
- **A test must not assume the template's own demo configuration is live.** A copy that has switched a feature off or replaced the example content is the normal case, not a broken one; a test that fails there is telling its owner to switch back on something they deliberately switched off. Gate such assertions with `{ skip: ... }` on the feature being present, the way `packages/template/tests/api-reference-config.test.mjs` and `packages/template/tests/private-leaks.test.mjs` do.
- **The hosted docs at <https://potluck-docs.vercel.app> are built from `apps/docs/`**, which renders `packages/template/wiki/` in place. Configuration material belongs there rather than in the template's `README.md`, so it has one home.
